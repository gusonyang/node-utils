...
var controller = {
     ...
     * 查询对象
     */
    pagination: function (params, filters, likes, order, attributes) {
        var where = {};
        //过滤属性
        if (filters) {
     ...
     * 查询单个
     */
    load: function (model, handle, attributes) {
        return function (params) {
            var data = {where: {id: params.id}};
            if (attributes) {
    ...
    edit: function (model, handle) {
        return function (body) {
            ...
            return model.update(body, {where: where}).then(function (result) {
                ...
                if (handle) handle({success: response.success, data: body});
                return response;
            });
        }
    },
     ...
     * 扩展管理员 api
     */
    extend_admin: function (actions, model, object_type, methods, handle, attributes) {
        actions.model = model;
        for (var i in methods) {
            var method = methods[i];
            actions[method] = controller[method](model, handle, attributes);
        }
