var controller = {
    ...
    extend_admin: function (actions, model, object_type, methods) {
        ...
        for (var i in methods) {
            var method = methods[i], action_type = false;
            if (method == 'load') {
                action_type = 'browse';
            } else if (method == 'sort') {
                action_type = 'edit';
            ...
            actions[method].check = (method == 'validate') ? false : {
                admin: true,
                permission: {object_type: object_type, action_type: action_type || method}
            }
        }
    },
     ...
     * @param methods
     */
    extend_store: function (actions, model, object_type, methods) {
        actions.model = model;
        for (var i in methods) {
            var method = methods[i], action_type = false;
            actions[method] = controller[method](model);
            actions[method].check = {
                store: true
            }
        }
    },
     ...
     * @param methods
     */
    extend_source: function (actions, model, object_type, methods) {
        actions.model = model;
        for (var i in methods) {
            var method = methods[i], action_type = false;
            actions[method] = controller[method](model);
