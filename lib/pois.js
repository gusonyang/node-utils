...
module.exports = function (config, utils) {
    ...
    var api = {
        ...
        get_poi: function (params, td) {
            ...
            return api.json(ele_host, params).then(function (result) {
                if (result.name === 'SERVER_UNKNOWN_ERROR') {
                    debug.err(new Error('SERVER_UNKNOWN_ERROR:' + JSON.stringify(params)));
                    if (!td) {
                        return api.get_poi(params, true);
                    }
                }
                return result;
            });
         ...
         * 获取一组地址
         */
        get_pois: function (params, keyword, td) {
            var loc = coords.bd2gcj(params.longitude, params.latitude);
            var hash = geohash.encode(loc[1], loc[0]);
            var data = {
                geohash: hash,
            ...
            return api.json(ele_host, params).then(function (rows) {
                ...
                if (rows.length) {
                    for (var i in rows) {
                        var row = rows[i];
                        if (i === 0 && !row.name) {
                            debug.err(new Error('SERVER_UNKNOWN_ERROR:' + JSON.stringify(params) + JSON.stringify(rows)));
                            if (!td) {
                                return api.get_pois(params, keyword, true);
                            }
                        }
